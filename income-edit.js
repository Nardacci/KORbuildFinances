...
async function init(){
 ...
 $('description').value=i.description||'';$('category').value=i.category||'';$('amount').value=i.amount??'';$('account').value=i.account_id||'';{const sel=$('currency');const opt=[...sel.options].find(o=>code(o.value)===code(i.currency||'BRL'));if(opt)sel.value=opt.value;}$('receipt-date').value=i.receipt_date||'';$('frequency').value=i.frequency||'eventual';$('status').value=i.status||'realized';
}
