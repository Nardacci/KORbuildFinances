...
async function init(){const session=await KORbuildAuth.session();if(!session?.user){location.replace('index.html');return}const {data,error}=await db().from('user_workspaces').select('id,display_name,country,primary_currency,setup_completed').eq('user_id',session.user.id).maybeSingle();if(error)throw error;if(!data||!data.setup_completed){location.replace('workspace.html');return}workspace=data;header(session.user);$('name').value=data.display_name||'';$('country').value=data.country||'';$('currency').value=data.primary_currency||'';
 const [gr,pr,ir]=await Promise.all([
  db().from('goals').select('id,name,target_amount,target_years,start_date').eq('workspace_id',workspace.id).order('created_at',{ascending:false}).limit(1).maybeSingle(),
 ...
 if(goal){$('goalName').value=goal.name||'';$('goalTarget').value=goal.target_amount??'';$('goalYears').value=goal.target_years??'';$('goalStartDate').value=goal.start_date||'';}
...
$('save').onclick=async()=>{const msg=$('message'),btn=$('save');btn.disabled=true;msg.className='save-message';msg.textContent='Salvando…';try{
 ...
 const goalName=$('goalName').value.trim(),goalTarget=Number($('goalTarget').value||0),goalYears=Number($('goalYears').value||0),goalStartDate=$('goalStartDate').value,planContribution=Number($('planContribution').value||0);
 ...
 if(goal)updates.push(db().from('goals').update({name:goalName,target_amount:goalTarget,target_years:goalYears,start_date:goalStartDate}).eq('id',goal.id));
 ...
 if(goal)goal={...goal,name:goalName,target_amount:goalTarget,target_years:goalYears,start_date:goalStartDate};
