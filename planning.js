...
async function load(){
 ...
 const now=new Date(),[start,end]=monthBounds(now),analysisStart=new Date(now.getFullYear(),now.getMonth()-11,1).toISOString().slice(0,10),primary=code(w.primary_currency||'BRL');
 const [{data:goals,error:ge},{data:plans,error:pe},{data:accounts,error:ae},{data:positions,error:pose},{data:tx,error:te},{data:expenses,error:ee},{data:incomes,error:ie}]=await Promise.all([
  ...
  db().from('investment_transactions').select('amount,currency,transaction_type,transaction_date').eq('workspace_id',w.id).gte('transaction_date',analysisStart).lt('transaction_date',end),
  db().from('expenses').select('amount,currency,status,paid_date').eq('workspace_id',w.id).gte('paid_date',analysisStart).lt('paid_date',end),
  db().from('incomes').select('amount,currency,status,receipt_date').eq('workspace_id',w.id).gte('receipt_date',analysisStart).lt('receipt_date',end)
 ...
 const ensureMonth=k=>{if(!buckets.has(k))buckets.set(k,{income:0,expense:0,investment:0})};
 inc.filter(x=>x.status==='realized'&&code(x.currency)===primary).forEach(x=>{const k=monthKey(x.receipt_date);ensureMonth(k);buckets.get(k).income+=Number(x.amount||0)});
 ex.filter(x=>x.status==='realized'&&code(x.currency)===primary).forEach(x=>{const k=monthKey(x.paid_date);ensureMonth(k);buckets.get(k).expense+=Number(x.amount||0)});
 itx.filter(x=>['contribution','adjustment'].includes(x.transaction_type)&&code(x.currency)===primary).forEach(x=>{const k=monthKey(x.transaction_date);ensureMonth(k);buckets.get(k).investment+=Number(x.amount||0)});
 const monthsWithData=[...buckets.values()].filter(m=>m.income!==0||m.expense!==0||m.investment!==0),dataMonths=monthsWithData.length;
 const sum=key=>monthsWithData.reduce((s,m)=>s+Number(m[key]||0),0),incomeTotal=sum('income'),expenseTotal=sum('expense'),investTotal=sum('investment');
 const divisor=Math.max(1,dataMonths),incomeAvg=incomeTotal/divisor,expenseAvg=expenseTotal/divisor,investAvg=investTotal/divisor,capacity=incomeAvg-expenseAvg;
 const required=requiredMonthly(currentWealth,target,months,rate),projected=futureValue(currentWealth,monthlyPlan,months,rate),realProjected=futureValue(currentWealth,Math.max(0,investAvg),months,rate),remaining=Math.max(0,target-currentWealth),progress=target?Math.min(100,Math.max(0,currentWealth/target*100)):0;
 ...
 const periodBadge=$('.period-badge');if(periodBadge)periodBadge.textContent=dataMonths===0?'Sem dados':dataMonths===1?'1 mês com dados':`${dataMonths} meses com dados`;
 const capacityLabel=$('.reality-grid .big-metric:first-child small'),investmentLabel=$('.reality-grid .big-metric:nth-child(2) small'),capacityText=dataMonths<=1?'Capacidade no mês':'Capacidade média de acumulação',investmentText=dataMonths<=1?'Investimento no mês':'Investimento médio';
 if(capacityLabel)capacityLabel.textContent=capacityText;if(investmentLabel)investmentLabel.textContent=investmentText;
 const realityNote=dataMonths===0?'Ainda não há movimentações suficientes para formar uma análise de realidade.':dataMonths===1?'A análise usa 1 mês de dados. À medida que novos lançamentos forem registrados, a média será recalculada automaticamente.':`A análise usa ${dataMonths} meses com dados dentro da janela móvel de até 12 meses.`;
 set('goal-name',goal.name||'Meu sonho');set('goal-description',`Meta de ${moneyFn(target)} em ${years||0} anos.`);set('target',moneyFn(target));set('current-wealth',moneyFn(currentWealth));set('remaining',moneyFn(remaining));set('progress',progress.toLocaleString('pt-BR',{maximumFractionDigits:1})+'%');
 set('real-capacity',moneyFn(dataMonths===1?capacity:capacity));set('real-investment',moneyFn(dataMonths===1?investAvg:investAvg));set('monthly-plan',moneyFn(monthlyPlan));set('required-monthly',moneyFn(required));set('required-gap',(monthlyPlan>=required?'+ ':'− ')+moneyFn(Math.abs(monthlyPlan-required)));set('plan-rate',rateText);
 ...
 set('month-income',moneyFn(incomeMonth));set('month-expenses',moneyFn(expenseMonth));set('month-invested',moneyFn(investMonth));set('reality-note',realityNote);
 ...
 projectionChart(currentWealth,target,monthlyPlan,Math.max(0,investAvg),months,moneyFn);
}
